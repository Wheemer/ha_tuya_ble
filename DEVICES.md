...
These inherited entity mappings are separate from local-pairing support. A listed device is not a guarantee that it can be provisioned locally. Hardware-verified local pairing currently covers the SGS01 soil sensor (product ID `gvygg3m8`, protocol 3.1). Experimental mappings need feedback from owners. See [setup and limitations](README.md#local-pairing-support).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td rowspan="4">Light support depends on the local product mappings and any metadata already saved with the device. This integration does not retrieve descriptions from the cloud. Upstream testing covered Strip Lights (category dd) and Magiacous RGB light bar (product ID nvfrtxlq).<br><br>See note on Bluetooth Mesh light compatibility below.</td>
    </tr>
    <tr>
